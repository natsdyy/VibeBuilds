      ...
      <main className="pt-40 pb-20 px-6 max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col items-center text-center mb-16">
          <motion.div 
            ...
            className="w-20 h-20 rounded-3xl bg-[#fd9a00]/10 flex items-center justify-center text-[#fd9a00] mb-8 border border-[#fd9a00]/20"
          >
            <Beaker className="w-10 h-10" />
          </motion.div>
          <h1 className="text-6xl md:text-8xl font-black tracking-tight mb-6">
            {t('labs.title').split('Demos')[0]}<span className="text-[#fd9a00]">Demos</span>
          </h1>
          <p className="text-[var(--text-muted)] text-xl max-w-2xl font-medium leading-relaxed">
            {t('labs.subtitle')}
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-20">
          <div className="p-2 bg-foreground/[0.03] backdrop-blur-xl border border-foreground/10 rounded-2xl flex flex-wrap gap-2">
            {categories.map((cat) => (
              <button
                key={cat.id}
                onClick={() => setActiveCategory(cat.id)}
                className={`relative px-6 py-3 rounded-xl text-sm font-black tracking-widest uppercase flex items-center gap-2 transition-all duration-300 ${
              ...
              >
                {activeCategory === cat.id && (
                  <motion.div
                    ...
                    className="absolute inset-0 bg-[#fd9a00] rounded-xl -z-10 shadow-lg shadow-[#fd9a00]/20"
                  ...
                  <div className="h-full p-10 rounded-[40px] border border-foreground/10 bg-foreground/[0.02] backdrop-blur-3xl hover:border-[#fd9a00]/30 transition-all duration-500 group-hover:-translate-y-2 flex flex-col overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-[#fd9a00]/5">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        {activeCategory === 'all' && (
                          <span className="text-[8px] font-black tracking-widest uppercase text-foreground/40 border border-foreground/10 px-2 py-1 rounded-md">
                            {exp.category}
                          </span>
                        )}
                      </div>
